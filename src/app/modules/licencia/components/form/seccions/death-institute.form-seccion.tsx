...
export const DeathInstituteFormSeccion: React.FC<IDeathInstituteProps<any>> = (props) => {
  ...
  return (
    <>
      ...
      >
        <Input allowClear placeholder='Número de Protocolo' autoComplete='off' disabled={!isMedicinaLegal} />
      </Form.Item>
      ...
      <Form.Item
        ...
        rules={[{ required: isMedicinaLegal, max: 10 }]}
      >
        <Input allowClear placeholder='Número de Acta de Levantamiento' autoComplete='off' disabled={!isMedicinaLegal} />
      </Form.Item>

      <Form.Item label='Fecha de Acta' name='instFechaActa' rules={[{ required: isMedicinaLegal }]}>
        <DatepickerComponent picker='date' dateDisabledType='before' dateFormatType='default' disabled={!isMedicinaLegal} />
      </Form.Item>

      <Form.Item label='Seccional Fiscalía' name='instSeccionalFiscalia' rules={[{ required: isMedicinaLegal, max: 5 }]}>
        <Input allowClear placeholder='Seccional Fiscalía' autoComplete='off' disabled={!isMedicinaLegal} />
      </Form.Item>

      <Form.Item label='Nombres y Apellidos del Fiscal' name='instNombreFiscal' rules={[{ required: isMedicinaLegal }]}>
        <Input allowClear placeholder='Nombres y apellidos completos' autoComplete='off' disabled={!isMedicinaLegal} />
      </Form.Item>

      <Form.Item label='No. Fiscal' name='instNoFiscal' rules={[{ required: isMedicinaLegal }]}>
        <Input allowClear type='tel' placeholder='No. Fiscal' autoComplete='off' disabled={!isMedicinaLegal} />
      </Form.Item>

      <Form.Item label='No. Oficio Medicina Legal' name='instNoOficioMedicinaLegal' rules={[{ required: isMedicinaLegal }]}>
        <Input allowClear type='tel' placeholder='No. Oficio Medicina Legal' autoComplete='off' disabled={!isMedicinaLegal} />
      </Form.Item>
