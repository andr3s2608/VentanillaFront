var utils = (function () {
  return {
    ...
    leaveSite: function leaveSite(url) {
      var message = 'Con esta acción abrirás una nueva pestaña. ¡Te esperamos pronto!';
      var opcAction = [
        {
          text: 'Cancelar',
    ...
    maintenanceSite: function maintenanceSite() {
      var message = 'Esta página esta en mantenimiento en breve estaremos de vuelta.<br><br>';
    ...
    errorSite: function errorSite() {
      var message = 'Puedes escribirnos a soporteccc@mintic.gov.co.<br>Llámanos gratis: 01 8000 910742 y en Bogotá: 3 90 79 51';
      var opcAction = [
        {
          text: 'Regresar',
          action: "utils.hideModal('alert-modal')",
    ...
    initAccesibilidad: function initAccesibilidad() {
      ...
      document.querySelector('.contrast-ref').addEventListener('click', function (e) {
        if (mood == 1) {
    ...
    viewportLink: function (href) {
      window.open(
        href,
        '',
        'resizable=no,status=no,location=no,toolbar=no,menubar=yes,fullscreen=no,scrollbars=no,dependent=no,width=500,height=600'
      );
    },
    ...
    collapseOnClick: function collapseOnClick(classComponent, idLoop) {
      ...
      if (state == 'false') {
        ...
        jQuery('.' + classComponent + ' .title-entity-' + classComponent + ':not(#title-entity-' + idLoop + ')').hide(
          200,
          function () {}
        );
    ...
    scrollHeader: function scrollHeader() {
      ...
      var itemFirstMenu = document.getElementsByClassName('nav-item-primary')[0];
    ...
    setConfigTable: function setConfigTable(configTable) {
      ...
      paginator_btn['next'] = '\n Siguiente';
      var numberOfDates = $('table#'.concat(configTable.idSelector, ' tbody tr')).length;
      ...
      var mergeLanguage = {
        language: utils.mergeObjects([defaultConfigTable.language, configTable.language])
      };
      var mergeConfig = utils.mergeObjects([defaultConfigTable, configTable, mergeLanguage]);
